...
def model_fn(features, labels, mode, params, config):
    ...
    embedding_size = params["embedding_size"] # 256(128+128)
...
def build_estimator():
    ...
    config = tf.estimator.RunConfig(
        model_dir="python_code\deep_neural_networks_for_youtube_recommendations\dnn\ckpt",
        tf_random_seed=2019,
...
def train_eval_save():
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(
            path='python_code\deep_neural_networks_for_youtube_recommendations\dnn\\tfrecords\\train\\train.tfrecords',
            parallel_num=32,
            epoch_num=11,
            batch_size=32),
        max_steps=1600
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(
            path='python_code\deep_neural_networks_for_youtube_recommendations\dnn\\tfrecords\evaluation\evaluation.tfrecords',
            parallel_num=32,
            epoch_num=1,
            batch_size=32),
        steps=15,     # 验证集评估多少批数据
    ...
    estimator.export_savedmodel(
        "python_code\deep_neural_networks_for_youtube_recommendation\dnn\modelpath",

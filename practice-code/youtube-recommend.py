...
def init_data(read_file):
    ...
    label_classes = len(set(df['item_id']))
    g = df.groupby(['user_id'], as_index=False)['item_id'].count()
    g.rename(columns={'item_id': 'user_item_count'}, inplace=True)
    top_user = g.sort_values(['user_item_count'])
    u = df.groupby(['item_id'], as_index=False)['user_id'].count()
    u.rename(columns={'user_id': 'item_user_count'}, inplace=True)
    top_item = u.sort_values(['item_user_count'])
    ...
    for line in f:
        ...
        if data[1] not in label_dict:
            label_dict[data[1]]= label_cnt
            label_cnt += 1

        if data[0] not in sku_dict:
            sku_dict[data[0]] = sku_cnt
            sku_cnt += 1
    return label_dict, sku_dict
...
label_dict, sku_dict = init_data(train_file)
